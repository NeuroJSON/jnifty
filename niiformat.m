...
## -*- texinfo -*-
## @deftypefn  {Function File} {@var{niiheader} =} niiformat ()
## @deftypefnx  {Function File} {@var{niiheader} =} niiformat (@var{format})
## Return a NIfTI header format descriptor as an Nx3 cell array. The input
## @var{format} can be either @code{'nifti1'} or @code{'nifti2'}. If
## @var{format} is not given, a @code{'nifti1'} header format descriptor is
## returned.
##
## The output @var{niiheader} is an N-by-3 cell array in the format similar to 
## the @code{'Format'} specifier of the @code{memmapfile} function in @sc{matlab}.
## The output @var{niiheader} can be used for @code{memmapstream} to parse a
## NIfTI header memory byte buffer.
##
## @var{niiheader} has the following structure:
